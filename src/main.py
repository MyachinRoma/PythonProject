...
from src.utils import financial_transactions, json_file_path
...
def main() -> None:
    ...
    for transaction in transactions:
        if "from" in transaction:
            from_ = get_mask_account_card(get_mask_account_card(transaction["from"]))
        else:
            from_ = "0"
        to_ = get_mask_account_card(get_mask_account_card(transaction["to"]))
